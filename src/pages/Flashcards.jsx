...
function Flashcards() {
  ...
  const [playingRecordingCardId, setPlayingRecordingCardId] = useState(null);
  ...
  const currentRecordingUrl = currentCard ? recordings[currentCard.id] : "";
  const isRecording = Boolean(recordingCardId);
  const isPlayingRecording = Boolean(playingRecordingCardId);
  const isAudioBusy = isListening || isSpeaking || isRecording || isPlayingRecording;
  ...
  }, [currentCard?.id, currentCard, hasStarted, partCheckpoint, isCompleted, isRecording, isPlayingRecording]);
  ...
  }, [currentCard?.id, currentCard, hasStarted, partCheckpoint, isCompleted, isLoading, isSpeaking, isListening, isRecording, isPlayingRecording]);
  ...
  const startRecording = async () => {
    ...
    if (isAudioBusy) {
      setStatusMessage("TTS, STT, 녹음, 녹음 듣기는 동시에 사용할 수 없습니다.");
      return;
    }

    if (!navigator.mediaDevices?.getUserMedia || !window.MediaRecorder) {
      setStatusMessage("이 브라우저는 목소리 녹음을 지원하지 않습니다.");
      return;
    }
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        setRecordings((previousRecordings) => {
          if (previousRecordings[cardId]) {
            URL.revokeObjectURL(previousRecordings[cardId]);
          }

          const nextRecordings = {
            ...previousRecordings,
            [cardId]: URL.createObjectURL(audioBlob),
          };
        ...
        setStatusMessage("녹음이 저장되었습니다. 녹음 듣기 버튼으로 확인할 수 있습니다.");
      };

      mediaRecorder.start();
      setRecordingCardId(cardId);
      setStatusMessage("목소리를 녹음하는 중입니다. 끝나면 녹음 중지 버튼을 눌러 주세요.");
    } catch (error) {
      console.error("녹음 시작 오류:", error);
      stopRecordingStream();
      setRecordingCardId(null);
      setStatusMessage("마이크 권한을 확인한 뒤 다시 녹음해 주세요.");
    }
  };

  const playRecording = () => {
    if (!currentCard || !currentRecordingUrl) {
      return;
    }
    ...
    setPlayingRecordingCardId(currentCard.id);
    setStatusMessage("녹음한 목소리를 재생합니다.");

    audio.onended = () => {
      setPlayingRecordingCardId(null);
      recordingAudioRef.current = null;
    };
    audio.onerror = () => {
      setPlayingRecordingCardId(null);
      recordingAudioRef.current = null;
      setStatusMessage("녹음 파일을 재생할 수 없습니다.");
    };
    audio.play().catch(() => {
      setPlayingRecordingCardId(null);
      recordingAudioRef.current = null;
  ...
  const startListening = ({ isAutoStart = false } = {}) => {
    ...
    if (isRecording || isPlayingRecording) {
      setStatusMessage("STT, 녹음, 녹음 듣기는 동시에 사용할 수 없습니다.");
      return;
    }
          ...
          {currentCard?.phase === "t2-translate" ? (
            <>
              <button
                ...
                type="button"
                onClick={recordingCardId === currentCard?.id ? stopRecording : startRecording}
                disabled={!currentCard || (isAudioBusy && recordingCardId !== currentCard?.id)}
              >
                {recordingCardId === currentCard?.id ? "녹음 중지" : "목소리 녹음"}
              </button>
              <button
                className="flashcard-secondary-button"
                type="button"
                onClick={playRecording}
                disabled={!currentRecordingUrl || isAudioBusy}
              >
                녹음 듣기
              </button>
              <button
                ...
                disabled={!currentCard || isAudioBusy}
              >
